...
class MDSPostProcessor(BasePostProcessor):
    ...
    def setup(self, net: nn.Module, id_loader_dict, ood_loader_dict):
        ...
        if self.postprocessor_args.alpha_list:
            ...
            # logistic regression for optimal data
...
def get_MDS_stat(model, train_loader, num_classes, feature_type_list, reduce_dim_list):
    ...
    feature_class = [[None for _ in range(num_classes)] for _ in range(num_layer)]
    ...
    feature_mean_list = [[np.mean(feature_by_class, axis=0) for feature_by_class in feature_by_layer] for feature_by_layer in feature_class]
